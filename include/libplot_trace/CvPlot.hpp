    ...
    namespace plot
    {
        ...
        class CV_EXPORTS_W Plot2d : public Algorithm
        {
            public:
                ...
                CV_WRAP virtual void setPlotLineColor(Scalar _plotLineColor, int idx) = 0;
                ...
                CV_WRAP virtual void setLegendStrings(std::vector<std::string> strings) = 0;
                /**
        ...
        class Plot2dImpl : public Plot2d
        {
            public:
                ...
                Plot2dImpl(InputArray plotDataX_, InputArray plotDataY_)
                {
                    ...
                    if ((_plotDataX.cols > 1 && _plotDataX.rows > 1) )
                        CV_Error(Error::StsBadArg, "ERROR: Plot data must be a 1xN or Nx1 matrix.\n");
                ...
                void setLegendStrings(vector<string> legend)
                {
                    _legend_str = legend;
                }
                ...
                void setPlotLineColor(Scalar _plotLineColor, int idx)
                {
                    plotLineColor[idx] = _plotLineColor;
                }
                ...
                void render(OutputArray _plotResult)
                {
                    ...
                    int ImageYzero = (int)InterpYdataFindZero.at<double>(NumVecElements, 3);
                    ...
                    if (needPlotLine)
                    {
                        ...
                        for (int j = 0; j < InterpYdata.cols; j++)
                        {
                            drawLegend(j);
                            Point p1;
                            p1.x = (int)InterpXdata.at<double>(0, 0);
                            p1.y = plotSizeHeight - (int)InterpYdata.at<double>(0, j);

                            for (int r = 1; r<InterpXdata.rows; r++)
                            {
                                Point p2;
                                p2.x = (int)InterpXdata.at<double>(r, 0);
                                p2.y = plotSizeHeight - (int)InterpYdata.at<double>(r, j);

                                line(plotResult, p1, p2, plotLineColor[j], plotLineWidth, 8, 0);
                    ...
                    else
                    {
                        for (int j = 0; j < InterpYdata.cols; j++)
                        {
                            ...
                            for (int r = 0; r<InterpXdata.rows; r++)
                            {
                                ...
                                circle(plotResult, p, 1, plotLineColor[j], plotLineWidth, 8, 0);
                            }
                        }
                    }
            ...
            protected:
                ...
                //legend
                vector<string> _legend_str;

                //colors of each plot element
                Scalar plotLineColor[4];
                ...
                void plotHelper(Mat _plotDataX, Mat _plotDataY)
                {
                    ...
                    plotDataY_plusZero = Mat::zeros(NumVecElements + 1, _plotDataY.cols, CV_64F);

                    for (int i = 0; i<NumVecElements; i++)
                    {
                        plotDataX_plusZero.at<double>(i, 0) = plotDataX.at<double>(i, 0);
                        for (int j = 0; j < _plotDataY.cols; j++)
                            plotDataY_plusZero.at<double>(i, j) = plotDataY.at<double>(i, j);
                    }
                    ...
                    plotMinY = MinY;
                    plotMaxY = MaxY;
//                    printf("%f, %f, %f, %f\n", plotMinX, plotMaxX, plotMinY, plotMaxY);
                    ...
                    setPlotAxisColor(Scalar(50, 50, 50));
                    setPlotGridColor(Scalar(150, 150, 150)); //setPlotGridColor(Scalar(255, 255, 255));
                    setPlotBackgroundColor(Scalar(250, 250, 250));//setPlotBackgroundColor(Scalar(0, 0, 0));
                    setPlotLineColor(Scalar(0, 0, 255  ), 0);
                    setPlotLineColor(Scalar(255, 0, 0  ), 1);
                    setPlotLineColor(Scalar(0, 128, 0  ), 2);
                    setPlotLineColor(Scalar(255, 0, 255), 3);
                ...
                void drawLegend(int idx)
                {
                    if (_legend_str.size() <= idx)
                        return;
                    double TextSize = 0.8;
                    putText(plotResult, _legend_str[idx].c_str(), Point(80, 40+idx*20), FONT_HERSHEY_COMPLEX_SMALL, TextSize, plotLineColor[idx], 1, 8);
                ...
                void drawAxis(int ImageXzero, int ImageYzero, double CurrentX, double CurrentY, Scalar axisColor, Scalar gridColor)
                {
                    if (needShowText)
                    {
//                        drawValuesAsText(plotMinX, 0, plotSizeHeight / 2, 10, 20);
                        drawValuesAsText(plotMaxX, plotSizeWidth, plotSizeHeight / 2, -60, 20);
                        double dg = (plotMaxY - plotMinY) / gridLinesNumber;
                        drawValuesAsText(plotMinY, 10, plotSizeHeight, 10, -5);
                        for (int i = 1; i < gridLinesNumber; i++)
                        {
                            char txt[8]{};
                            sprintf(txt, "%.2f", plotMinY + dg * i);
                            putText(plotResult, txt, Point(10, plotSizeHeight - i * plotSizeHeight / gridLinesNumber), FONT_HERSHEY_COMPLEX_SMALL, 0.8, plotTextColor, 1, 8);
                        }
                        drawValuesAsText(plotMaxY, 0, 0, 10, 20);
...
//                        drawValuesAsText((format("Y_%d = ", cursorPos) + "%g").c_str(), CurrentY, 0, 20, 40, 20);
                    ...
                    int TraceSize = 5;
                    drawLine(0, plotSizeWidth, plotSizeHeight/2, plotSizeHeight/2, axisColor);
                ...
                Mat linearInterpolation(double Xa, double Xb, double Ya, double Yb, Mat Xdata)
                {
                    ...
                    for (int i = 0; i<Xdata.rows; i++)
                    {
                        ...
                        {
                            double X = Xdata.at<double>(i, j);

                            Ydata.at<double>(i, j) = int(Ya + (Yb - Ya)*(X - Xa) / (Xb - Xa));

                            if (Ydata.at<double>(i, j)<0)
                                Ydata.at<double>(i, j) = 0;
                        }
